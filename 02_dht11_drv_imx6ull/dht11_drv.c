...
static ssize_t dht11_read (struct file *file, char __user *buf, size_t size, loff_t *offset)
{
    ...
    mdelay(20);                   // 拉低 20ms，确保可靠

    /* 3. 释放总线（拉高）并切换为输入 */
    gpio_direction_input(gpios[0].gpio);  // 上拉电阻将引脚拉高

    /* 4. 注册中断，等待DHT11响应 */
    err = request_irq(gpios[0].irq, dht11_isr, IRQF_TRIGGER_RISING | IRQF_TRIGGER_FALLING,
                      gpios[0].name, &gpios[0]);
    if (err) {
        printk(KERN_ERR "request_irq failed, err=%d\n", err);
        gpio_free(gpios[0].gpio);
        return err;
    }

    /* 5. 设置超时定时器（20个jiffies，约200ms） */
    mod_timer(&gpios[0].key_timer, jiffies + msecs_to_jiffies(200));
    ...
    del_timer_sync(&gpios[0].key_timer);  // 确保定时器不会再次触发
